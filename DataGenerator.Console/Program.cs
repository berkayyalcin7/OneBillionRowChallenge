// 413 farklı şehire göre veri üreteceğiz.
...
const string DefaultOutputFilePath = GlobalConstants.FilePath;
...
Console.WriteLine("╔══════════════════════════════════════════════════════════════╗");
Console.WriteLine("║           1BRC Weather Data Generator (.NET 10)              ║");
Console.WriteLine("╚══════════════════════════════════════════════════════════════╝");
Console.WriteLine();
...
Console.Write($"  Çıktı dosyası [{DefaultOutputFilePath}]: ");
var pathInput = Console.ReadLine();
var outputPath = string.IsNullOrWhiteSpace(pathInput) ? DefaultOutputFilePath : pathInput;

Console.WriteLine();
Console.WriteLine($"  ► Satır sayısı:   {rowCount:N0}");
Console.WriteLine($"  ► Çıktı dosyası:  {outputPath}");
Console.WriteLine($"  ► İstasyon sayısı: {Stations.Length}");
Console.WriteLine();
...
// Pre-allocate station data for faster access
var stations = Stations;
var stationCount = stations.Length;

// Use a thread-local random for better performance
...
using (var writer = new StreamWriter(fileStream, utf8NoBom,BufferSize))
{
    ...
    for (long row = 0; row < rowCount; row++)
    {
        ...
        // Generate temperature with Gaussian-like distribution (Box-Muller approximation using uniform)
        
        // Simple approach: mean +/- random variance up to 10 degrees with more values near mean
        ...
        // Update progress
        // Progress is updated every ProgressInterval rows to avoid excessive console output. 10.000.000 satırda bir güncelleme yapacağız.
        completedRows = row + 1;
...
Console.WriteLine($"  Rows/second:    {rowCount / stopwatch.Elapsed.TotalSeconds:N0}");
...
if (fileInfo.Exists)
{
    ...
    Console.WriteLine($"  File size:      {sizeGB:F2} GB ({fileInfo.Length:N0} bytes)");
}
